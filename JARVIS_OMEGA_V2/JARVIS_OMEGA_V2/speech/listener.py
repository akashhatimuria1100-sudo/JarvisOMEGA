"""
speech/listener.py — JARVIS OMEGA V2 Voice Listener (FINAL FIXED)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
FIXES:
  ✅ MAX_RECORD_S = 180 (3 minutes)
  ✅ listen() timeout = 180.0 seconds
  ✅ VAD_THRESHOLD = 0.35 (better detection)
  ✅ SILENCE_S = 2.0 (doesn't cut off mid-sentence)
  ✅ Energy threshold lowered to 0.008
"""
...
class VoiceListener:
    ...
    CHANNELS = 1
    CHUNK_FRAMES = 512
    MAX_RECORD_S = 180          # FIXED: 3 minutes
    SILENCE_S = 2.0             # FIXED: 2 seconds silence
    VAD_THRESHOLD = 0.35        # FIXED: better detection
    ...
    def _init_vad(self):
        try:
            ...
            logger.warning("Silero-VAD not available: %s", exc)
    ...
    def listen(self, timeout: float = 180.0) -> str:
    ...
    def listen_continuous(self, callback, stop_event: threading.Event = None):
        ...
        while not stop.is_set():
            text = self.listen(timeout=180.0)
    ...
    def _listen_whisper(self, timeout: float) -> str:
        ...
        try:
            ...
            logger.info("Listening (Whisper+VAD)... timeout=%.0fs", timeout)
            while True:
                if time.time() - start_time > timeout:
                    logger.info("Listen timeout reached (%.0fs)", timeout)
                    break
