"""
tools/tool_manager.py — JARVIS OMEGA V4 Tool Dispatcher
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
FIXES vs V3:
  ✅ focus_app — brings target window to front before typing/clicks
  ✅ type_text checks active window to avoid typing in JARVIS itself
...
class ToolManager:
    ...
    def execute(self, action: str, params: dict) -> dict:
        try:
            ...
            if action == "focus_app":         return self._focus_app(params)
            # App control
    ...
    def _focus_app(self, params: dict) -> dict:
        """Bring a target app window to the front before typing."""
        app = params.get("app", "").lower().strip()
        try:
            if OS == "Windows":
                try:
                    ...
                    return {"type": "error", "error": "pywin32 not installed. Run: pip install pywin32"}
                ...
                def _callback(hwnd, extra):
                    if win32gui.IsWindowVisible(hwnd):
                        ...
                        if target in title.lower():
                            extra.append(hwnd)
                handles = []
                win32gui.EnumWindows(_callback, handles)
                if handles:
                    hwnd = handles[0]
                    ...
                    time.sleep(0.5)
                    return {"type": "focused", "app": app, "title": win32gui.GetWindowText(hwnd)}
                else:
                    return {"type": "error", "error": f"Window for '{app}' not found. Open it first."}
            elif OS == "Linux":
                subprocess.run(["xdotool", "search", "--name", app, "windowactivate"], timeout=3)
                return {"type": "focused", "app": app}
            elif OS == "Darwin":
                subprocess.run(["osascript", "-e", f'tell application "{app}" to activate'], timeout=3)
                return {"type": "focused", "app": app}
        except Exception as exc:
            return {"type": "error", "error": f"Focus failed: {exc}"}
        return {"type": "error", "error": f"Focus not supported on {OS} for {app}"}
    ...
    def _type_text(self, params: dict) -> dict:
        """
        Type text using pyautogui, but FIRST verify we are not typing into JARVIS.
        If JARVIS is the active window, warn the user to click the target app first.
        """
        try:
            import pyautogui
            ...
            # Safety check: ensure not typing into JARVIS itself
            active = get_active_window_title().lower()
            jarvis_names = ["jarvis", "omega", "python", "pyqt", "qt"]
            if any(n in active for n in jarvis_names):
                # Try to focus the previously opened app if specified
                target_app = params.get("target_app", "")
                if target_app:
                    focus_result = self._focus_app({"app": target_app})
                    if focus_result.get("type") == "focused":
                        time.sleep(0.5)
                    else:
                        return {"type": "error", "error": f"JARVIS is the active window. Click the target app '{target_app}' first, then retry."}
                else:
                    return {"type": "error", "error": "JARVIS is the active window. Please click the target app first, then retry."}
